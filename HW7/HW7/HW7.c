...
int main()
{
	printf("Program by Dieter Steinhauser\n");

		// demonstrate the use of countEvenPositive() by using an array that contains 2,1,-5,0,8,-8 of length 6. 
		// Display the array contents and show the result of countEvenPositive() 
		...
		// Display the added array which should be 5, 8, 11, 14
			int test2[] = { 1, 2, 3, 4 };
			int test3[] = { 4, 6, 8, 10 };
			int test4[4];addArrays(test2, test3, test4, 4);
...
// in a3. The addition is performed element by element. Example:// the length is 4.
...
void reverseArray(int a1[], int length)
{	//create a new array to store the inverse
	int arrayRev[5];
	int indexRev = length - 1;
	int index = 0;
	while ( index < length)
	{
		arrayRev[index] = a1[indexRev - index];
		index++;
	}
	//assign all values to new array
	int i = 0;
	while ( i < length)
	{
		a1[i] = arrayRev[i];
...
// The countLetters() function will count the number of small letters from a to z and capital letter from A to Z in the a1 array. 
// The count will be the returned value of the countLetters() function.
...
int countLetters(char a1[])
{
	...
	while (a1[index] != '\0')
	{
		if ((a1[index] >= 'a'&& a1[index] <= 'z') || (a1[index] >= 'A'&& a1[index] <= 'Z'))
			letterCounter++;
			index++;
	}
...
// initializeWordString() function will count the number of characters in the base character array which is null terminated. The null terminator does not count as a character. 
//The function will insert the dash character ('-') in the word character
...
// of the letterToSearch. If letterToSearch is contained at least 1 time in the array then the searchString() function will return a 1, otherwise searchString() will return a 0.
...
// word character array a lower case letter. The word character array is null terminated.
